...
function imagesArtistInfo() {
  $(".c-img-contain-item").each(function () {
    ...
    let headlineClosest = $(this)
      .closest(".c-section")
      .find(".t-display-1.is-animated");
    let heroLogoClosest = $(this)
      .closest(".c-section")
      .find(".c-img-contain.hero-logo");
...
function teamCardsAnimation() {
  if ($(".c-team-item").length > 0) {
    $(".c-team-item").each(function () {
      ...
      let noPhotoItem = $(this).has(
        ".c-img-contain.team.w-condition-invisible"
      );
...
// --- VIDEOS ELEMENTS
let heroVideo = $(".c-video.hero video").get(0);
let footerVideo = $(".c-video.footer video").get(0);

// --- HERO LOGO ANIMATION
function heroLogoAnimation(video) {
  ScrollTrigger.create({
    trigger: ".c-video.hero",
    start: "top center",
    end: "bottom 60%",
    onEnter: () => video.play(),
...
function footerLogoAnimation(video) {
  ScrollTrigger.create({
...
function homeLoader() {
  ...
  tl.call(heroLogoAnimation, [heroVideo], "<0.3");
}
...
function subPageLoader() {
  ...
  let headlineText1Value = parseFloat(
    headlineText1.attr("headline-sub-line-1")
  );
  let headlineText2Value = parseFloat(
    headlineText2.attr("headline-sub-line-2")
  );

  gsap.set(".c-img-contain.sub-hero", { autoAlpha: 1 });
  gsap.set(".c-img-contain.sub-hero .c-img", {
...
function init() {
  ...
  headerStatus();
  fade();
  footerLogoAnimation(footerVideo);
  if (homePage) {
    hideLogoHome();
    homeLoader();
  }
